...
<div class="min-h-screen bg-[#fff7ed]">
    ...
    <!-- Header -->
    <header class="bg-[#ffedd5] border-b border-gray-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center py-4 gap-4">

                <div>
                    <h1 class="text-xl sm:text-2xl font-bold text-[#E65100]">Dinasti Sushi</h1>
                    <p class="text-xs sm:text-sm text-gray-700">Sistem Rekomendasi AI</p>
                </div>

                <div class="flex items-center space-x-4 sm:space-x-6 text-sm sm:text-base">

                    <a href="{{ route('menu.index') }}" 
                       class="flex items-center text-black hover:text-black transition-colors">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if (session('success'))
            <div class="bg-green-50 border border-green-200 text-green-700 px-6 py-4 rounded-2xl mb-6 flex items-center">
                <svg class="w-6 h-6 mr-3 shrink-0" fill="currentColor" viewBox="0 0 20 20">
        ...
        <!-- Welcome Banner -->
        <div class="bg-linear-to-r from-[#F57C00] to-[#E65100] rounded-2xl sm:rounded-3xl shadow-xl p-6 sm:p-8 mb-6 sm:mb-8 text-black">
        ...
        <!-- CF Section -->
        <div class="bg-[#fff7ed] rounded-2xl sm:rounded-3xl shadow-lg p-6 sm:p-8 mb-8" x-data="{ open: false }">
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <div class="flex items-center">
                    <svg class="w-6 h-6 sm:w-8 sm:h-8 text-[#E65100] mr-3" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clip-rule="evenodd"/>
                    </svg>
                    <h3 class="text-lg sm:text-2xl font-bold text-[#E65100]">Algoritma Collaborative Filtering</h3>
                </div>

                <button @click="open = !open" 
                        class="text-[#F57C00] hover:text-[#E65100] font-medium flex items-center">
                    <span x-text="open ? 'Sembunyikan Detail' : 'Lihat Detail'"></span>
                    <svg class="w-5 h-5 ml-1 transform transition-transform" :class="{ 'rotate-180': open }" fill="currentColor"
                        viewBox="0 0 20 20">
        ...
        <div class="mb-8">
            <div class="flex items-center mb-6">
                <svg class="w-6 h-6 sm:w-8 sm:h-8 text-[#FF8C00] mr-3" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
                <h3 class="text-lg sm:text-2xl font-bold text-[#E65100]">Rekomendasi Khusus Untuk Anda</h3>
            </div>

            @if(count($recommendations) > 0)
                @foreach($recommendations as $rec)
                <div class="bg-[#fff7ed] rounded-2xl sm:rounded-3xl shadow-lg p-4 sm:p-6 lg:p-8 mb-6">
                    <div class="flex flex-col md:flex-row items-start gap-6">

                        <div class="relative shrink-0 w-full md:w-64">
                            <img src="{{ $rec['menu_item']->image }}" 
                                 class="w-full md:w-64 h-48 object-cover rounded-2xl">
                            <span class="absolute top-4 left-4 
                                         bg-[#F57C00] text-white px-3 sm:px-4 py-1 
                        ...
                        <div class="grow w-full">
                            <div class="flex flex-col sm:flex-row justify-between items-start gap-4 mb-4">
                                ...
                                <div class="grow">
                                    <h4 class="text-xl sm:text-2xl font-bold text-[#E65100] mb-2">
                                        {{ $rec['menu_item']->name }}
                                    </h4>

                                    <span class="inline-block bg-[#ffedd5] text-[#E65100] px-3 py-1 
                                                 rounded-full text-sm mb-3">
                                    ...
                                    <p class="text-2xl sm:text-3xl font-bold text-[#F57C00]">
                                        Rp {{ number_format($rec['menu_item']->price, 0, ',', '.') }}
                                    </p>
                                </div>

                                <a href="{{ route('menu.show', $rec['menu_item']->id) }}" 
                                   class="w-full sm:w-auto bg-linear-to-r 
                                          from-[#F57C00] to-[#E65100] 
                                          text-white font-semibold px-6 py-3 rounded-xl 
                                          hover:opacity-90 transition-all text-center whitespace-nowrap">
                                    Lihat Detail
                                </a>
                            ...
                            <div class="grid grid-cols-3 gap-2 sm:gap-4 mt-6">
                                ...
                                <div class="bg-pink-50 rounded-xl p-2 sm:p-4 text-center">
                                    <div class="text-lg sm:text-2xl font-bold text-[#E65100]">
                                        {{ $rec['menu_item']->rating_count }}
                                    </div>
                ...
                <div class="bg-[#fff7ed] rounded-3xl shadow-lg p-8 text-center">
                    ...
                    <a href="{{ route('menu.index') }}" 
                       class="inline-block bg-linear-to-r from-[#F57C00] to-[#E65100] 
                              text-white font-semibold px-6 py-3 rounded-xl hover:opacity-90">
                        Lihat Menu
                    </a>
                </div>
            @endif
